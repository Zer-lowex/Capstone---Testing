                        ...
                        <table id="productTable" class="table table-striped table-bordered text-center align-middle">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('supplier.view') }}?supplier_id={{ $product->supplier->id ?? '' }}" class="supplier-link">
...
<script>
     $('#deleteModal').on('show.bs.modal', function (event) {
        ...
        form.attr('action', '/admin/products/delete/' + productId);
    });
</script>
...
<script>
    ...
    $('#productSearch').on('keyup', function() {
        ...
        if (search === '') {
            $.ajax({
                url: '{{ route('product.view') }}', // Using product.view route
                type: 'GET',
                    ...
                    products.forEach(function(product) {
                        ...
                        tableData += `
                            <tr>
                                ...
                                <td>
                                    ${product.supplier_id ? 
                                        `<a href="{{ route('supplier.view') }}?supplier_id=${product.supplier_id}" class="supplier-link">
                                            ${product.supplier?.name || 'No Supplier'}
                                        </a>`
                                        : 'No Supplier'
                                    }
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <!-- Edit Button -->
                                        <div class="d-inline">
                                            <a href="/admin/products/edit/${product.id}" class="btn btn-success btn-sm me-2">
                                                <i class="bx bxs-edit"></i>
                                            </a>
                                        </div>
                                        <!-- Add Stock Button -->
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.length > 0) {
                        ...
                        tableData = `<tr><td colspan="10">No results found</td></tr>`;
                    }
    ...
    $(document).on('click', '[data-bs-target="#addStockModal"]', function (event) {
        ...
        $.ajax({
            url: '{{ route('admin.products.filter') }}', // Adjust this to your filter route
            type: 'GET',
    ...
    $('#resetFilter').on('click', function () {
        ...
        // Reload all products
        $.ajax({
            url: '{{ route('product.view') }}', // Adjust this to your default product list route
            type: 'GET',
            success: function (response) {
                let products = response.data; // Access products array
                let tableData = '';
