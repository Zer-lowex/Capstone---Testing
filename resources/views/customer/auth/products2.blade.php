                ...
                <div class="modal-body p-4">
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4 mb-md-0">
                            <div class="ratio ratio-1x1 bg-light rounded-3 overflow-hidden d-flex align-items-center justify-content-center">
                        ...
                        <div class="col-md-6">
                            ...
                            <!-- Category Badge -->
                            <span id="modalProductCategory" class="badge bg-primary mb-3"></span>
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <div>
                                    ...
                                    <span id="modalProductPrice" class="fs-4 fw-bold text-primary ms-2"></span>
                                    <small class="text-muted d-block">(₱<span id="unitPrice">0.00</span> per <span id="modalProductUnit">pc</span>)</small>
                                </div>
                                <div class="text-end">
                                    <span class="text-muted">Stock:</span>
                                    <span id="modalProductStock" class="fw-bold ms-2"></span>
                                </div>
                            </div>
                            ...
                            {{-- <div class="mb-4">
                                ...
                                <div class="input-group" style="max-width: 150px;">
                                    <button class="btn btn-outline-secondary" id="decrementQty" type="button"
                                    ...
                                    <button class="btn btn-outline-secondary" id="incrementQty" type="button"
                            ...
                            <!-- Reserve Button -->
                            @auth('customer')
                            <form method="POST" action="{{ route('customer.reserve.product') }}">
                                @csrf
                                <input type="hidden" name="product_id" id="modalProductId">
                                <div class="mb-3">
                                    <label for="modalProductQuantity" class="form-label">Quantity</label>
                                    <input type="number" name="quantity" id="modalProductQuantity" class="form-control" value="1" min="1" required>
                                </div>
                                <button type="submit" class="btn btn-primary w-100 py-2">
                                    <i class="bx bx-cart-add me-2"></i> Reserve Product
                                </button>
                            </form>
                            @else
                                <a href="{{ route('customer.login') }}" class="btn btn-outline-primary w-100 py-2">
                                    <i class="bx bx-log-in-circle me-2"></i> Login to Reserve Product
                                </a>
                            @endauth
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            productModal.addEventListener('show.bs.modal', function (event) {
                ...
                document.getElementById('modalExpiredAt').value = expiredAt.toISOString();
                document.getElementById('modalProductQuantity').max = productQuantity;
                ...
                document.getElementById('unitPrice').textContent = parseFloat(productPrice).toFixed(2);
                document.getElementById('modalProductUnit').textContent = productUnit || 'Not Available';
                document.getElementById('modalProductDescription').textContent = productDescription || 'Not Available';
                document.getElementById('modalProductCategory').textContent = productCategory || 'Uncategorized';
                document.getElementById('modalProductImage').src = productImage || '{{ asset("images/Product Image Not Available.jpg") }}';
                document.getElementById('modalProductStock').textContent = productQuantity + ' ' + (productUnit || '');

                // Add warning icon if stock is low
                if (parseInt(productQuantity) <= 5) {
                    const warningIcon = document.createElement('i');
                    warningIcon.className = 'bx bx-error-alt text-danger ms-1';
                    document.getElementById('modalProductStock').appendChild(warningIcon);
                }
            });
        });
    </script>
