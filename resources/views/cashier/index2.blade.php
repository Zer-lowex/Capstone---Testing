                        ...
                        <div class="card p-3">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="discount_type" id="discountType" class="form-select"
                                    onchange="handleDiscountChange(this)">
                        ...
                        if (response.success) {
                            ...
                            if (shouldPrint && response.printReceipt) {
                                ...
                                setTimeout(function() {
                                    ...
                                    var printWindow = window.open('/cashier/receipts2/' +
                                        response.printReceipt, 'ReceiptPrint',
                                        windowFeatures);

                                    if (!printWindow) {
                                        toastr.warning(
                                            'Popup was blocked. Please enable popups for this site to print receipts.'
                                        );
                                        return;
                                    }
                                    ...
                                    printWindow.onload = function() {
                                        try {
                                            ...
                                            console.log(
                                                "Automatic printing blocked by browser");
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.length === 0) {
                            $('#suggestions').html(
                                '<li class="list-group-item">No Products Found</li>').show();
                        } else {
                    ...
                    error: function() {
                        $('#suggestions').html(
                                '<li class="list-group-item">Error fetching products</li>')
                            .show();
                    }
                });
            });
            ...
            function calculateChange() {
                // Get the payment amount (parse as float, default to 0 if empty/NaN)
                let paidAmount = parseFloat($('#paid_amount').val()) || 0;

                // Get the total amount from the hidden input
                let totalAmount = parseFloat($('#finalAmountInput').val()) || 0;

                // Calculate change
                let change = paidAmount - totalAmount;

                // Update the change field (show 2 decimal places)
